...
const jobRefetchJeonsePriceIndexAptAroundSeoul = schedule.scheduleJob(
  refetchRule,
  function () {
    console.log(
      "현재시간 토요일 04시 00분 수도권 아파트 전세 지수 데이터 리패치를 진행합니다."
    );
    try {
      axios
        .get(
        ...
        .then((response) => {
          const truncateQuery = `TRUNCATE jeonse_price_index_apt_around_seoul;`;
          db.query(truncateQuery, (err, result) => {
            if (err)
              return console.log(
                "수도권 아파트 전세 지수 : TRUCATE에 실패 했습니다."
              );
            console.log(
              "수도권 아파트 전세 지수 : TRUCATE성공, 새 데이터 추가를 진행합니다."
            );
            ...
            for (const item of response.data) {
              ...
              db.query(
                ...
                (err, result) => {
                  if (err)
                    return console.log(
                      "수도권 아파트 전세 지수 : 새 데이터 추가에 실패 했습니다."
                    );
                  ...
                  if (insertCount === response.data.length) {
                    //=====================================================================================
                    const message =
                      "수도권 아파트 전세 지수 : 모든 데이터를 새 데이터로 교체 완료하였습니다.";
                    ...
                    db.query(
                      ...
                      (err, result) => {
                        if (err)
                          return console.log(
                            "수도권 아파트 전세 지수 : 업데이트 공지 테이블에 추가하지 못했습니다."
                          );
                        console.log(
                          "수도권 아파트 전세 지수 : 새 데이터로 교체 완료하였습니다."
                        );
                      }
                    );
                    //=====================================================================================
...
module.exports = jobRefetchJeonsePriceIndexAptAroundSeoul;
