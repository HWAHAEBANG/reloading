...
const jobRefetchHousePriceIndexAptSeoul = schedule.scheduleJob(
  refetchRule,
  function () {
    console.log(
      "현재시간 토요일 04시 00분 서울 아파트 매매 지수 데이터 리패치를 진행합니다."
    );
    try {
      axios
        .get(
        ...
        .then((response) => {
          const truncateQuery = `TRUNCATE house_price_index_apt_seoul;`;
          db.query(truncateQuery, (err, result) => {
            if (err)
              return console.log(
                "서울 아파트 매매 지수 : TRUCATE에 실패 했습니다."
              );
            console.log(
              "서울 아파트 매매 지수 : TRUCATE성공, 새 데이터 추가를 진행합니다."
            );
            ...
            for (const item of response.data) {
              ...
              db.query(
                ...
                (err, result) => {
                  if (err)
                    return console.log(
                      "서울 아파트 매매 지수 : 새 데이터 추가에 실패 했습니다."
                    );
                  ...
                  if (insertCount === response.data.length) {
                    //=====================================================================================
                    const message =
                      "서울 아파트 매매 지수 : 모든 데이터를 새 데이터로 교체 완료하였습니다.";
                    ...
                    db.query(
                      ...
                      (err, result) => {
                        if (err)
                          return console.log(
                            "서울 아파트 매매 지수 : 업데이트 공지 테이블에 추가하지 못했습니다."
                          );
                        console.log(
                          "서울 아파트 매매 지수 : 새 데이터로 교체 완료하였습니다."
                        );
                      }
                    );
                    //=====================================================================================
...
module.exports = jobRefetchHousePriceIndexAptSeoul;
