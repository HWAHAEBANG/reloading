...
const jobUpdateHousePriceIndexAptAroundSeoul = schedule.scheduleJob(
  updateRule,
  function () {
    console.log(
      "현재시간 00시 00분 수도권 아파트 매매 지수 데이터 최신화를 진행합니다."
    );
    try {
      // DB의 가장 최신 데이터의 날짜와 값 가져오기
      const checkSqlQuery =
        "SELECT origin_date,value FROM house_price_index_apt_around_seoul WHERE no = (SELECT MAX(no) FROM house_price_index_apt_around_seoul);";
          ...
          .then((response) => {
            ...
            if (latestDataApi.date === latestDataDb.date) {
              if (latestDataApi.value === latestDataDb.value) {
                console.log(
                  "수도권 아파트 매매 지수 : 현재 DB는 최신 상태 입니다. "
                );
              } else {
                const updateSqlQuery = `UPDATE house_price_index_apt_around_seoul SET value = ? WHERE origin_date = ?`;
                db.query(
                  updateSqlQuery,
                  ...
                  (err, result) => {
                    ...
                    //=====================================================================
                    const message =
                      "수도권 아파트 매매 지수 : 최근 일자의 데이터가 수정되었습니다.";
                    ...
                    db.query(
                      ...
                      (err, result) => {
                        ...
                        console.log(
                          "수도권 아파트 매매 지수 : 데이터에 변경사항이 감지되어 DB를 수정하였습니다."
                        );
                      }
                    );
                    //=====================================================================
              ...
              db.query(
                ...
                (err, result) => {
                  ...
                  //========================================================================
                  const message =
                    "수도권 아파트 매매 지수 : 새로운 데이터가 추가되었습니다.";
                  const notificationQuery = `INSERT INTO data_update_logs (message,update_type) VALUES (?,?);`;
                  db.query(
                    notificationQuery,
                    [message, "add"],
                    (err, result) => {
                      ...
                      console.log(
                        "수도권 아파트 매매 지수 : 새로운 데이터가 감지되어 DB에 추가하였습니다."
                      );
                    }
                  );
                  //========================================================================
...
module.exports = jobUpdateHousePriceIndexAptAroundSeoul;
