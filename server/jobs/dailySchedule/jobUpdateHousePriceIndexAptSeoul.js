...
const jobUpdateHousePriceIndexAptSeoul = schedule.scheduleJob(
  updateRule,
  function () {
    console.log(
      "현재시간 00시 00분 서울 아파트 매매 지수 데이터 최신화를 진행합니다."
    );
    try {
      // DB의 가장 최신 데이터의 날짜와 값 가져오기
      const checkSqlQuery =
        "SELECT origin_date,value FROM house_price_index_apt_seoul WHERE no = (SELECT MAX(no) FROM house_price_index_apt_seoul);";
          ...
          .then((response) => {
            ...
            if (latestDataApi.date === latestDataDb.date) {
              if (latestDataApi.value === latestDataDb.value) {
                console.log(
                  "서울 아파트 매매 지수 : 현재 DB는 최신 상태 입니다. "
                );
              } else {
                const updateSqlQuery = `UPDATE house_price_index_apt_seoul SET value = ? WHERE origin_date = ?`;
                db.query(
                  updateSqlQuery,
...
module.exports = jobUpdateHousePriceIndexAptSeoul;
