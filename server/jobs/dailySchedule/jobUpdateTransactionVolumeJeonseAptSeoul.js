...
const jobUpdateTransactionVolumeJeonseAptSeoul = schedule.scheduleJob(
  updateRule,
  function () {
    console.log(
    ...
    try {
      ...
      const checkSqlQuery =
        "SELECT origin_date,value FROM transaction_volume_jeonse_apt_seoul WHERE no = (SELECT MAX(no) FROM transaction_volume_jeonse_apt_seoul);";
          ...
          .then((response) => {
            ...
            if (latestDataApi.date === latestDataDb.date) {
              if (latestDataApi.value === latestDataDb.value) {
                console.log(
                  "서울 아파트 전세 거래량 : 현재 DB는 최신 상태 입니다. "
                );
                //==============================================================================================================
                // 기존 데이터 모두 지우기
                const truncateQuery = `TRUNCATE transaction_volume_jeonse_apt_seoul;`;
                db.query(truncateQuery, (err, result) => {
                  if (err)
                    return console.log(
                      "서울 아파트 전세 거래량 : TRUCATE에 실패 했습니다."
                    );
                  console.log(
                    "서울 아파트 전세 거래량 : TRUCATE성공, 새 데이터 추가를 진행합니다."
                  );
                  ...
                  for (const item of insertData) {
                    ...
                    db.query(
                      ...
                      (err, result) => {
                        if (err)
                          return console.log(
                            "서울 아파트 전세 거래량 : 새 데이터 추가에 실패 했습니다."
                          );
                        ...
                        if (insertCount === insertData.length) {
                          console.log(
                            "서울 아파트 전세 거래량 : 새 데이터로 교체 완료하였습니다."
                          );
                        }
                      }
                    );
                ...
                console.log(
                  "서울 아파트 전세 거래량 : 데이터에 변경사항이 감지되어 전체 데이터를 리셋합니다. "
                );
                //==============================================================================================================
                // 기존 데이터 모두 지우기
                const truncateQuery = `TRUNCATE transaction_volume_jeonse_apt_seoul;`;
              ...
              console.log(
                "서울 아파트 전세 거래량 : 새로운 데이터가 감지되어 전체 데이터를 리셋합니다."
              );
              //==============================================================================================================
              // 기존 데이터 모두 지우기
              const truncateQuery = `TRUNCATE transaction_volume_jeonse_apt_seoul;`;
...
module.exports = jobUpdateTransactionVolumeJeonseAptSeoul;
...
//                 );
//               } else {
//                 const updateSqlQuery = `UPDATE transaction_volume_jeonse_apt_seoul SET value = ? WHERE origin_date = ?`;
//                 db.query(
//                   updateSqlQuery,
...
//                     console.log(
//                       "서울 아파트 전세 거래량 : 데이터에 변경사항이 감지되어 DB를 수정하였습니다."
//                     );
//                   }
//                 );
//               }
...
//                   if (err) return console.log(err);
//                   console.log(
//                     "서울 아파트 전세 거래량 : 새로운 데이터가 감지되어 DB에 추가하였습니다."
//                   );
//                 }
//               );
//             }
